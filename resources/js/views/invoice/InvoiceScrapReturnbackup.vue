<template>
    <div class="container-fluid" id="spare-parts">
        <breadcrumb :options="['Scrap Product Return']"></breadcrumb>
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                                    ...
                                    <tr v-for="(invD,i) in invoiceDetails" :key="i">
                                        ...
                                        <td>
                                            <ValidationProvider name="Reason" mode="eager"
                                                                rules="required"
                                                                v-slot="{ errors }">
                                                <select class="form-control" v-model="invD.Reason">
                                        ...
                                        <td>
                                            <ValidationProvider name="Quantity" mode="eager"
                                                                :rules="`required|min_value:1|max_value:${invD.Quantity}`"
                                                                v-slot="{ errors }">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-danger btn-sm"
                                                    @click="removeFieldsRow(i)">Remove
                                            </button>
                                        </td>
                                    </tr>
                                </table>
...
export default {
    ...
    methods: {
        changeQuantityOrDiscount(i) {
            console.log('er')
            let price = Number(this.invoiceDetails[i].UnitPrice) * Number(this.invoiceDetails[i].rQuantity)
            this.invoiceDetails[i].TotalAmount = price
        },
        onSubmit() {
            this.axiosPost('invoice-spare-parts/get-scrap-product-data-by-id', {
                invoiceNo: this.invoiceNo
            }, (response) => {
                if (response.invoiceMaster && response.invoiceDetails) {
                    this.invoiceMaster = response.invoiceMaster
                    this.invoiceDetails = response.invoiceDetails
                }
                else{
                    this.errorNoti('No Data Found')
                }
        ...
        removeFieldsRow(id) {
            this.invoiceDetails.splice(id, 1)
        },
        onSubmitReturn() {
            if (this.invoiceDetails.length > 0) {
                this.axiosPost('invoice-spare-parts/scrap-products-return', {
