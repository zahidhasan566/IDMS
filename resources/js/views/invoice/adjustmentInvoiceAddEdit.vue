<template>
    <div class="container-fluid">
        <breadcrumb :options="{title}">
            <router-link class="btn btn-primary" :to="{name:'adjustmentInvoiceIndex'}">Back</router-link>
        </breadcrumb>
        <div class="row" v-if="actionType==='add'">
            <div class="col-12 col-md-6">
                <ValidationProvider name="Invoice Adjustment File" mode="eager" rules="required"
                    ...
                    <label style="font-weight:bold" for="inputExcelFile">* Invoice Adjustment File (Excel) <span
            ...
            <div class="col-md-3">
                <label style="font-weight:bold" for="downloadExcelFile">Sample Invoice Adjustment File: <span
                        class="error">*</span></label>
        ...
        <div class="row" style="padding-top: 15px" v-if="form.ExcelData.importStatus">
            <div class="row col-md-12">
                ...
                <div class="col-md-6">
                    <div class="row">
                        <div class="col-md-4" style="text-align: end;padding-top: 10px;"><p>MasterCode</p></div>
                        <div class="col-md-8">
                                <div class="form-group">
                                    <multiselect v-model="form.masterCode"
                                                 :options="allDealer"
                                                 :multiple="false"
                                                 :disabled="actionType==='edit'?true:false"
                                                 @search-change="getSupportingData"
                                    ...
                                    <span class="error"
                                          v-if="errors[form.masterCode] !== undefined && errors[form.masterCode] !== undefined">{{
                                            errors[form.masterCode]
                                        }}</span>
                                </div>
                                <span class="error-message"> {{ errors[0] }}</span>
                        </div>
...
export default {
    ...
    mounted() {
        if (this.$route.query.action_type === 'edit') {
            this.title= 'Return Adjustment';
            this.actionType = 'edit'
            this.buttonText = 'Return'
            let adjustmentInvoiceNo  = this.decodeConvert(this.$route.query.adjustmentInvoiceNo);
            let instance = this

            //Get Existing Data
            this.axiosGet('invoice-spare-parts/get/existing-adjustment/' + adjustmentInvoiceNo, function (response) {
                //instance.returnExcelData=response.data
                instance.form.ExcelData =  response.data
                instance.form.adjustmentInvoiceNo = response.data[0].AdjustmentInvoiceNo
                instance.form.masterCode= {
                    'CustomerCode':response.data[0].MasterCode,
                    'CustomerName':response.data[0].MasterCode+ '-'+  response.data[0].CustomerName
                }
                instance.form.ExcelData.importStatus= true
               console.log( instance.form.ExcelData)
    ...
    methods: {
        ...
        downloadDemoExcel() {
            axios.get(baseurl + "api/invoice-spare-parts/export-adjustment-demo-excel", this.config()).then((res) => {
                const downloadAnchor = document.createElement("a");
                downloadAnchor.setAttribute("href", res.data);
                downloadAnchor.setAttribute("download", "adjustment_file_upload_format.xls");
        ...
        onSubmit() {
            ...
            if (this.actionType === 'add') {
                ...
                submitUrl = 'api/invoice-spare-parts/adjustment-store';
            }
            if (this.actionType === 'edit') {
                submitUrl = 'api/invoice-spare-parts/adjustment-return';
            }
            if(this.errors.length === 0){
                $("#submitAdjustInvoice").hide();
                this.form.post(baseurl + submitUrl, this.config()).then(response => {
                    if(response){
                        this.successNoti(response.data.message);
                        this.form.ExcelData = []
                        if (this.actionType === 'edit') {
                            this.$router.push({name: 'adjustmentInvoiceIndex',})
                        }
                        else{
