<template>
  <div class="container-fluid">
    <breadcrumb :options="['Job Card Report']">
        ...
        <div class="card">
          <div class="datatable" v-if="!isLoading">
            <div class="card-body">
                    ...
                    <div class="col-md-2">
                      ...
                      <div class="col-md-2">
                          ...
                          <div class="form-group">
                              ...
                              <select name="jobStatus" class="form-control" v-model="form.CustomerCode" style="margin: 0">
                                  ...
                                  <option :value="singleCustomer.value" v-for="(singleCustomer , index) in customer"
                                          :key="index">{{singleCustomer.value}}- {{ singleCustomer.text }}</option>
                              </select>
                      ...
                      <div class="col-md-2">
                          <div class="form-group">
                              ...
                              <select name="jobTypes" class="form-control" v-model="form.JobType" style="margin: 0">
                                  <option value="">Select</option>
                                  <option :value="jobType.value" v-for="(jobType , index) in jobTypes" :key="index">{{ jobType.text }}</option>
                              </select>
                              <div class="error" v-if="form.errors.has('jobTypes')" v-html="form.errors.get('jobTypes')" />
                          </div>
                      </div>
                      <div class="col-md-2">
                          <div class="form-group">
                              <label>Job Status</label>
                              <select name="jobStatus" class="form-control" v-model="form.JobStatus" style="margin: 0">
                                  <option value="">Select</option>
                                  <option :value="singleJobStatus.value" v-for="(singleJobStatus , index) in jobStatus" :key="index">{{ singleJobStatus.text }}</option>
                              </select>
                              <div class="error" v-if="form.errors.has('jobStatus')" v-html="form.errors.get('jobStatus')" />
                          </div>
                      </div>
              ...
              <div v-if="contents.length > 0">
                <div class="table-responsive">
                  <table class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small">
                    ...
                    <tr>
                        <th>CSI</th>
                      <th style="background: #333547" v-for="(item, index) in headers" v-if="index !== 0 && index !== 33">
                    ...
                    <tr v-for="(item, index) in contents" >
                        <td>
                            <button class="btn btn-primary" style="font-size: 12px;width:65px;padding: 2px 0px"  @click="addCSIModal(item)">CSI</button>
                        </td>
                      <td v-for="(item2, index) in headers.slice(1)" v-bind:class="isInt(item[item2]) === true ? 'text-right' : '' " v-if="index !== 33">
                        {{ item[item2] }}
                      </td>
                    </tr>
                    </tbody>
              ...
              <add-edit-jobCard-csi  v-if="csiLoading"/>
          </div>
          <div v-else>
...
export default {
  ...
  mounted() {
    ...
    bus.$on('add-edit-jobCard-csi-data-refresh', (item) => {
        this.filterAllReport();
    })

  },
    destroyed() {
        bus.$off('add-edit-jobCard-csi-data-refresh')
    },
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/jobCard/job-card-csi-data", this.config()).then(response => {
      ...
      addCSIModal(item){
        this.csiLoading = true
          setTimeout(() => {
              bus.$emit('add-edit-jobCard-csi', item);
          })
      },
    exportReport(){
      // this.form.Export = 'Y';
      // this.form.post(baseurl + "api/jobCard/job-card-report", this.config()).then(response => {
      ...
      //     bus.$emit('data-table-import', dataSets, columns, 'Job Card Report Export')
      //   }
      // }).catch(e => {
      //   //
      // });
      ...
      this.form.post(baseurl + "api/jobCard/job-card-csi-data", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          link.href = url;
          link.setAttribute('download', 'Job_Card_Report.csv');
    ...
    getSupportingData() {
          let instance = this;
          this.axiosGet('jobCard/job-report-supporting-data', function (response) {
              instance.customer = instance.arrayKeyChange(response.user)
              instance.jobStatus = response.tblJobStatus
              instance.jobTypes = response.allJobType
