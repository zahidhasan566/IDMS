<template>
  <div class="container-fluid">
    <breadcrumb :options="['Claim Warranty List']">
      <router-link :to="{name: 'ClaimWarrantyCreate'}" class="btn btn-primary btn-sm">Add Claim Warranty</router-link>
      <button type="button" class="btn btn-success btn-sm" @click="exportWarrantyClaim">Export</button>
    </breadcrumb>
    <div class="row">
      <div class="col-sm-12">
        <div class="card">
              ...
              <div class="d-flex">
                <div class="flex-grow-1">
                  <ValidationObserver v-slot="{ handleSubmit }">
                    <form @submit.prevent="handleSubmit(getAllClaimWarranty)" @keydown.enter="$event.preventDefault()">
                      <div class="row">
                        <div class="col-md-2">
...
<!--                        </router-link>-->
                        <router-link :to="`${mainOrigin}approval/warranty-print/${item.dcwarrantyid}`" style="height: 18px;padding: 0px 3px 18px 3px;" class="btn btn-info btn-sm">
...
export default {
  ...
  mounted() {
    document.title = 'Warranty List | DMS';
  ...
  methods: {
    getAllClaimWarranty(){
      this.isLoading = true;
      this.form.Export = '';
      this.form.post(baseurl + 'api/get-all-claim-warranty', this.config() ).then((response)=>{
    ...
    exportWarrantyClaim(){
      axios.get(baseurl + 'api/get-all-claim-warranty' , this.config() )
          .then((response)=>{
            let dataSets = response.data.warranty;
            if (dataSets.length > 0) {
              ...
              bus.$emit('data-table-import', dataSets, columns, 'Warranty Claim Export')
            }
          }).catch((error)=>{
      })
    },
    searchData(){
      axios.get(baseurl + "api/search/ceiling/" + this.query + "?page=" + this.pagination.current_page, this.config() ).then(response => {
        this.ceilings = response.data.data;
