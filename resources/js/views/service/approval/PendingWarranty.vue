<template>
  <div class="container-fluid">
    <breadcrumb :options="['Pending Warranty List']">
              ...
              <div class="d-flex">
                <div class="flex-grow-1">
                  <ValidationObserver v-slot="{ handleSubmit }">
                    <form @submit.prevent="handleSubmit(getAllPendingWarranty)" @keydown.enter="$event.preventDefault()">
                      <div class="row">
                        <div class="col-md-2">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small">
                    ...
                    <tr v-for="(item, index) in contents" >
                      ...
                      <td>
                        <router-link :to="`pending-warranty-edit-and-approved/${item.Warranty_Id}`" class="btn btn-info btn-sm">
                          Edit & Approve
                        </router-link>
                        <button @click="destroy(item.Warranty_Id)" style="height: 18px;padding: 0px 3px 18px 3px;" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                      </td>
                    </tr>
                    </tbody>
                  </table>
                ...
                <div class="row">
                  ...
                  <div class="col-8">
                    <report-pagination
                        ...
                        :offset="5"
                        @paginate="form.Query === '' ? getAllPendingWarranty() : getAllPendingWarranty()"
                    ></report-pagination>
                  </div>
                </div>
              </div>
...
export default {
  ...
  mounted() {
    document.title = 'Pending Warranty List | DMS';
    this.getAllPendingWarranty();
  },
  methods: {
    getAllPendingWarranty(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/get-all-pending-warranty", this.config()).then(response => {
    ...
    destroy(id){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.get(baseurl + 'api/claim-warranty-delete/'+ id, this.config()).then((response)=>{
            this.getAllPendingWarranty();
            Swal.fire(
                'Deleted!',
