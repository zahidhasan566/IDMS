<template>
  <div class="container-fluid">
    <breadcrumb :options="['Claim Warranty Edit & Approved']">
      <router-link :to="{name: 'PendingWarranty'}" class="btn btn-primary btn-sm">Back</router-link>
    </breadcrumb>
    <div class="row">
      <div class="col-sm-12">
        <div class="card">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="row m-b-15">
                        ...
                        <div class="col-12 col-md-4">
                          <div class="form-group row" style="padding-bottom: 10px">
                            <label class="col-lg-4 col-form-label text-right">Slod Date</label>
                            <div class="col-lg-8">
                              <input name="Color" type="text" class="form-control" readonly v-model="form.SlodDate" />
                            </div>
                          </div>
                        </div>
                        <div class="col-12 col-md-4">
                          <div class="form-group row" style="padding-bottom: 10px">
                            <label class="col-lg-4 col-form-label text-right">Dealer Name</label>
                            <div class="col-lg-8">
                              <input name="Color" type="text" class="form-control" readonly v-model="form.DealerName" />
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="row form-divider m-b-15">
                        ...
                        <div class="col-12 col-md-4" v-for="(image, index) in form.Picture" :key="index">
                          <p>Photo {{++index}}</p>
                          <img :src="showImage(image.ProImagePath)" height="200px" width="200px" alt="Selected Image" style="display: flex">
                        </div>
                      </div>
                      <br>
...
export default {
  ...
  data() {
    return {
      ...
      form: new Form({
        ...
        SourceOfInformation : '',
        Mileage : 0,
        ServiceSchedule : 'Not Applicable',
        TypeOfWarranty : '',
        Seriousness : '',
        TechnicianName : '',
        ProblemName : '',
  ...
  created() {
    axios.get(baseurl + `api/pending-warranty-edit-and-approved/${this.$route.params.WarrantyId}`, this.config()).then((response)=>{
      console.log(response)
      this.getWarrentyFirstTime();
      this.form.fill(response.data.warranty);
  ...
  mounted() {
    document.title = 'Claim Warranty Edit & Approved | DMS';
  },
  methods: {
    onSubmit(){
      this.PreLoader = true;
      this.form.post(baseurl + "api/claim-warranty-update", this.config()).then(response => {
        if (response.data.status === 'success'){
          this.$toaster.success(response.data.message);
          this.redirect(this.mainOrigin + 'approval/pending-warranty')
        }else {
    ...
    showImage(image){
      ...
      //return window.location.origin + "/dms-v2/assets/images/claim_warranty/" + image;
    },
    removeImage(index){
