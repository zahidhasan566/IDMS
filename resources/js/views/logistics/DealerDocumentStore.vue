                      ...
                      <div class="row form-divider m-b-15">
                        ...
                        <div class="col-12 col-md-4">
                          <div class="form-group">
                            ...
                            <input type="file"  @change="onFileChange" ref="fileInput" accept="application/pdf" multiple>
                          </div>
                          <ul>
    ...
    onFileChange(e) {
      ...
      onSubmit() {
          if (this.files.length === 0) {
              this.errorNoti("Please select at least one file.");
              return;
          }

          let formData = new FormData();
          this.files.forEach((file) => {
              formData.append("files[]", file);  // Ensure "files[]" matches Laravel backend
          });

          // Debugging: Log FormData before sending
          for (let pair of formData.entries()) {
              console.log(pair[0], pair[1]);  // Should log "files[]" and the file objects
          }

          axios
              .post(baseurl + "api/logistics/dealer-document", formData, this.config())
              .then((response) => {
              ...
              .catch((error) => {
                  console.error(error.response.data); // Show backend error response
