<template>
    <div class="container-fluid">
      <breadcrumb :options="['Dealer CSI Summary']">
  ...
  export default {
    ...
    methods: {
      filterAllReport(){
        this.isLoading = true
        this.form.Export = '';
        this.form.post(baseurl + "api/reports/customer-csi-summary", this.config()).then(response => {
      ...
      exportReport(){
        ...
        this.form.post(baseurl + "api/reports/customer-csi-summary", this.config()).then(response => {
          ...
          if (dataSets.length > 0) {
            ...
            bus.$emit('data-table-import', dataSets, columns, 'Dealer CSI Summary')
            this.exportShow = false;
          }
        }).catch(e => {
          //
