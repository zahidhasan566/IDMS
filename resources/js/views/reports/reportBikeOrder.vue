<template>
  <div class="container-fluid">
    <breadcrumb :options="['Bike Order Report']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/bike-order", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/bike-order", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          link.href = url;
          link.setAttribute('download', 'Bike_Order_Report.csv');
