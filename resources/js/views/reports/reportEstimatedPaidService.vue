<template>
    <div class="container-fluid">
      <breadcrumb :options="['Estimated Paid Service Report']">
                            ...
                            <div class="form-group">
                                ...
                                <select name="region" class="form-control" v-model="form.RegionName" style="margin: 0">
                                  <option :value="null" selected>None</option>
                                    <option :value="singleCustomer.RegionName" v-for="(singleCustomer , index) in region"
                                            :key="index">{{ singleCustomer.RegionName }} - {{ singleCustomer.RegionName }}</option>
                                </select>
  ...
  export default {
    ...
    methods: {
      filterAllReport(){
        this.isLoading = true
        this.form.Export = '';
        this.form.post(baseurl + "api/reports/estimated-paid-service", this.config()).then(response => {
      ...
      exportReport(){
        ...
        this.form.post(baseurl + "api/reports/estimated-paid-service", this.config()).then(response => {
          ...
          if (dataSets.length > 0) {
            ...
            bus.$emit('data-table-import', dataSets, columns, 'Estimated Paid Service Report')
            this.exportShow = false;
          }
        }).catch(e => {
          //
