<template>
    <div class="container-fluid">
      <breadcrumb :options="['Service Ratio']">
  ...
  export default {
    ...
    methods: {
      filterAllReport(){
        this.isLoading = true
        this.form.Export = '';
        this.form.post(baseurl + "api/reports/service-ratio", this.config()).then(response => {
      ...
      exportReport(){
        ...
        this.form.post(baseurl + "api/reports/service-ratio", this.config()).then(response => {
          ...
          if (dataSets.length > 0) {
            ...
            bus.$emit('data-table-import', dataSets, columns, 'Service Ratio')
            this.exportShow = false;
          }
        }).catch(e => {
          //
