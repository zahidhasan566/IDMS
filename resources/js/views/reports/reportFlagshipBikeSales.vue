<template>
  <div class="container-fluid">
    <breadcrumb :options="['Flagship Bike Sales']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/flagship-bike-sales", this.config()).then(response => {
    ...
    exportReport(){
      ...
      exportReportCSV(){
          ...
          this.form.post(baseurl + "api/reports/flagship-bike-sales", this.config()).then(response => {
              ...
              link.href = url;
              link.setAttribute('download', 'flagship_bike_sales_data.csv');
