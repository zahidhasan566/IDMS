<template>
  <div class="container-fluid">
    <breadcrumb :options="['Service Summary Report']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/service-summary", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/service-summary", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          link.href = url;
          link.setAttribute('download', 'Service-Summary_Report.csv');
