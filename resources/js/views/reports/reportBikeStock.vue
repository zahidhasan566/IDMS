<template>
  <div class="container-fluid">
    <breadcrumb :options="['Bike Stock Report']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/bike-stock", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/bike-stock", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          bus.$emit('data-table-import', dataSets, columns, 'Bike Stock Report')
          this.exportShow = false;
        }
      }).catch(e => {
        //
