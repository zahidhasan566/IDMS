<template>
  <div class="container-fluid">
    <breadcrumb :options="['Spare Parts Sales Report']">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small">
                    ...
                    <tr v-for="(item, index) in contents" >
                      ...
                      <td>
                        <router-link class="btn btn-primary" style="font-size: 10px;width:65px;padding: 2px 0px" target='_blank'
                                     :to="{name: 'InvoiceSparePartsPrint',params: {invoiceId:item.Invoice_Id}}">
                          <i class="fa fa-print">Print</i></router-link>
                      </td>
                    </tr>
                    </tbody>
                  </table>
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/spare-parts-sales", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/spare-parts-sales", this.config()).then(response => {
          ...
          link.href = url;
          link.setAttribute('download', 'spare_parts_sales_data.csv');
