<template>
  <div class="container-fluid">
    <breadcrumb :options="['Invoice Receive Survey Report']">
              ...
              <div v-if="contents.length > 0">
                <div class="table-responsive">
                  <table class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small">
                    ...
                    <tr>
                      <th style="background: #333547" v-for="(item, index) in headers" v-if="index !== 0">
                        <span v-if="index === 1 || index === 2 || index === 3 || index === 4 || index === 5 || index === 6">{{ item.replace(/_/g, ' ', " $1").trim() }}</span>
                        <span v-if="index === 7"> {{invoiceReceiveSurveyQuestion[0].SurveyQuestion}} </span>
                        <span v-if="index === 8"> {{invoiceReceiveSurveyQuestion[1].SurveyQuestion}} </span>
                        <span v-if="index === 9"> {{invoiceReceiveSurveyQuestion[2].SurveyQuestion}} </span>
                        <span v-if="index === 10"> {{invoiceReceiveSurveyQuestion[3].SurveyQuestion}} </span>
                        <span v-if="index === 11"> {{invoiceReceiveSurveyQuestion[4].SurveyQuestion}} </span>
                      </th>
                    </tr>
                    </thead>
                    <tbody>
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/invoice-survey-report-data", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/invoice-survey-report-data", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          bus.$emit('data-table-import', dataSets, columns, 'Invoice Receive Survey Report')
        }
      }).catch(e => {
        //
      });
    ...
    getSupportingData() {
          let instance = this;
          this.axiosGet('reports/invoice-receive-survey-report-supporting-data', function (response) {
              instance.customer = instance.arrayKeyChange(response.customer)
              instance.invoiceReceiveSurveyQuestion = response.invoiceReceiveSurveyQuestion
          }, function (error) {
          });
      },
    isInt(value) {
