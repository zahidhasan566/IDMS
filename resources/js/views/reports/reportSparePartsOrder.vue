<template>
  <div class="container-fluid">
    <breadcrumb :options="['Spare Parts Order Report']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/spare-parts-order", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/spare-parts-order", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          bus.$emit('data-table-import', dataSets, columns, 'Spare Parts Order Report')
          this.exportShow = false;
        }
      }).catch(e => {
        //
