<template>
  <div class="container-fluid">
    <breadcrumb :options="['Spare Parts Stock Report']">
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/reports/spare-parts-stock", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/reports/spare-parts-stock", this.config()).then(response => {
        ...
        if (dataSets.length > 0) {
            ...
            link.href = url;
            link.setAttribute('download', 'Spare_Parts_Stock_data.csv');
