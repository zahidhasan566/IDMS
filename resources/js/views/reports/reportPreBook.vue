<template>
  <div class="container-fluid">
    <breadcrumb :options="['Pre Book Report']">
                ...
                <form @submit.prevent="handleSubmit(filterAllReport)" @keydown.enter="$event.preventDefault()">
                  <div class="row">
                    <div class="col-md-2">
                      ...
                      <div class="col-md-2">
                              <div class="form-group">
                                  <label>Bike <span class="error">*</span></label>
                                  <select name="jobStatus" class="form-control" v-model="form.ProductCode" style="margin: 0">
                                      <option :value="null" v-if="user == 'admin'" selected>All</option>
                                      <option :value="singleProduct.ProductCode" v-for="(singleProduct , index) in products"
                                              :key="index">{{ singleProduct.ProductCode }} - {{ singleProduct.ProductName }}</option>
                                  </select>
                                  <div class="error" v-if="form.errors.has('CustomerCode')" v-html="form.errors.get('CustomerCode')" />
                              </div>
...
export default {
  ...
  methods: {
    filterAllReport(){
      this.isLoading = true
      this.form.Export = '';
      this.form.post(baseurl + "api/prebook/prebook-report-data", this.config()).then(response => {
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/prebook/prebook-report-data", this.config()).then(response => {
          ...
          link.href = url;
          link.setAttribute('download', 'pre_booking_data.csv');
