                ...
                <div class="row">
                  <div :class="options.colSize[0]" v-if="options.showFilter && options.showFilter.includes('startDate')">
                    <div>
                      ...
                      <datepicker v-model="requestParams.startDate" :dayStr="dayStr" placeholder="YYYY-MM-DD" :firstDayOfWeek="0"/>
                    </div>
                  </div>
                  <div :class="options.colSize[1]" v-if="options.showFilter && options.showFilter.includes('endDate')">
                    <label>End Date</label>
                    <datepicker v-model="requestParams.endDate" :dayStr="dayStr" placeholder="YYYY-MM-DD" :firstDayOfWeek="0"/>
                  </div>
                  <div :class="options.colSize[2]" v-if="options.showFilter && options.showFilter.includes('customers')">
                    ...
                    <select class="form-control" v-model="requestParams.customer">
                      ...
                      <option v-for="(c,index) in customers" :value="c.CustomerCode">{{c.CustomerName}}</option>
                    </select>
                  </div>
                    <div :class="options.colSize[2]" v-if="options.showFilter && options.showFilter.includes('products')">
                        <label>Products</label>
                        <v-select :filterable="false"
                                  :mutiselect="false"
                                  v-model="requestParams.product"
                                  :options="products" label="ProductName"
                                  @search="searchProduct">

                        </v-select>
<!--                        <select class="form-control" v-model="requestParams.product">-->
<!--                            <option value="">Select</option>-->
<!--                            <option v-for="(p,index) in products" :value="p.productcode">{{p.productname}}</option>-->
<!--                        </select>-->
                    </div>
                  ...
                  <div :class="options.colSize[4]" v-if="options.showFilter && options.showFilter.includes('business')">
                    ...
                    <select class="form-control" v-model="requestParams.business">
                      <option value="">Select</option>
                      <option v-for="(bus,index) in business" :value="bus.Business">{{bus.BusinessName}}</option>
                    </select>
                  </div>
                  <div :class="options.colSize[5]" v-if="options.showFilter && options.showFilter.includes('department')">
                    ...
                    <select class="form-control" v-model="requestParams.department">
                      <option value="">Select</option>
                      <option v-for="(dep,index) in department" :value="dep.ResStaffDepartment">{{dep.ResStaffDepartment}}</option>
                    </select>
                  </div>
                  <div :class="options.colSize[4]" v-if="options.showFilter && options.showFilter.includes('poNumber')">
                    <label>PO Number</label>
                    <input type="text" class="form-control" placeholder="Po Number" v-model="requestParams.poNumber">
                  </div>
                  <div :class="options.colSize[5]" v-if="options.showFilter && options.showFilter.includes('supplierId')">
                    <label>Supplier</label>
                    <input type="text" class="form-control" placeholder="Supplier ID" v-model="requestParams.supplierId">
                  </div>
                  <div :class="options.colSize[6]" v-if="options.showFilter && options.showFilter.includes('paymentMode')">
                    ...
                    <select class="form-control" v-model="requestParams.paymentMode">
                      <option value="">Select</option>
                      <option v-for="(mode,index) in paymentModes" :value="mode.PaymentMode">{{mode.PaymentModeName}}</option>
                    </select>
                  </div>
                  <div :class="options.colSize[7]" v-if="options.showFilter && options.showFilter.includes('bank')">
                    <label>Bank</label>
                    <select class="form-control" v-model="requestParams.bank">
                      <option value="">Select</option>
                      <option v-for="(bank,index) in banks" :value="bank.BankID">{{bank.BankName}}</option>
                    </select>
                  </div>
                  <div :class="options.colSize[8]" v-if="options.showFilter && options.showFilter.includes('closingDate')">
                    <label>Closing Date</label>
                    <datepicker v-model="requestParams.closingDate" :dayStr="dayStr" placeholder="YYYY-MM-DD" :firstDayOfWeek="0"/>
                  </div>
                </div>
...
export default {
  ...
  props: ['options','advance','business','department','paymentModes','banks','action','customers','products'],
  mixins: [Common],
  data() {
    return {
      time1: null,
  ...
  methods: {
    ...
    readData(type = '') {
      ...
      this.axiosPost(this.options.source + '?page=' + this.currentPage, this.requestParams,
          (response) => {
            if (type !== 'export') {
              this.dataSets = response.data[0];
              if (this.dataSets.length > 0) {
                let columns = Object.keys(response.data[0][0]);
                ...
                this.totalCount = response.data[1][0].CountData;
              ...
              let dataSets = response.data[0];
              if (dataSets.length > 0) {
                let columns = Object.keys(dataSets[0]);
                let allColumns = []
                // FILTER HIDDEN COLUMN START
                if (this.options.hideColumn) {
                  columns.forEach((item) => {
                    let find = this.options.hideColumn.find((hide) => hide == item);
                    if (!find) {
                ...
                //FILTER HIDDEN COLUMN END
                columns = allColumns.filter((item) => item !== 'row_num');
                let rex = /([A-Z])([A-Z])([a-z])|([a-z])([A-Z])/g;
