                ...
                <div class="row">
                  ...
                  <div :class="options.colSize[0]" v-if="options.showFilter && options.showFilter.includes('roles')">
                    ...
                    <select class="form-control" v-model="requestParams.role">
                      <option value="">Select</option>
                      <option v-for="(role,index) in roles" :value="role.RoleId">{{role.RoleName}}</option>
                    </select>
                  </div>
...
export default {
  ...
  props: ['options','action','roles','customers'],
  mixins: [Common],
  data() {
    return {
      time1: null,
  ...
  mounted() {
    ...
    this.requestParams.category = this.options.category;
