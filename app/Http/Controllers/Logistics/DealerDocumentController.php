        ...
        try {
            if ($request->file('files')) {
                foreach ($request->file('files') as $file) {
                    ...
                    $docNo = pathinfo($filename, PATHINFO_FILENAME); // Extract filename without extension

                    $existDealer = DealerDocument::where('DocumentSLNo', '=', $docNo)->first();

                    if (empty($existDealer)) {
                        $dealer = new DealerDocument();
                        $dealer->DocumentFileName = $filename;
                        $dealer->DocumentSLNo = $docNo;
                        ...
                        $existDealer->DocumentSLNo = $docNo;
                ...
                return response()->json([
                    'status' => 'error',
                    'message' => 'No files received from the request'
                ], 400);
            }
        } catch (\Exception $e) {
            return response()->json([
